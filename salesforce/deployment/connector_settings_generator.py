...
def generate_yaml_files(
    ...
    Args:
        df (pd.DataFrame): Pipeline configuration dataframe with columns:
            ...
            - project_name: Project name (required) - used for grouping when separate_dabs_per_project=True
        ...
        separate_dabs_per_project (bool): If True, creates separate DAB package for each project_name
            in the dataframe. Each project gets its own directory with databricks.yml and resources/.
            If False, uses the project_name parameter for a single DAB package (default: False)
    ...
    Example:
        # Single DAB package for all projects
        generate_yaml_files(df, 'my_project', 'output', targets)

        # Separate DAB packages per project (df must have project_name column)
        generate_yaml_files(df, 'my_project', 'output', targets, separate_dabs_per_project=True)
